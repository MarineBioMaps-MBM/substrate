---
title: "bioregion_norcal_substrate_analysis"
format: html
editor: visual
---
...
```{r}
# Filter to Northern CA
north_sub <- substrate |>
  filter(pmep_region == "Pacific Northwest")
```

```{r, warnings=FALSE}
# Join Northern CA to substrate
north_sub_mpa <- st_intersection(mpa, north_sub)
```

```{r}
# Drop geometry
north_sub_mpa <- north_sub_mpa %>%
  st_drop_geometry() %>%
  drop_units()
```
...
mpa_area_summary <- north_sub_mpa %>%
