---
title: "scsr_substrate"
format: html
editor: visual
---

SCSR stands for South Coast Study Region, as defined by the California Department of Fish and Wildlife.
...
total_scsr_ha_sub <- sum(scsr_substrate$area_ha)
# Check statement
print(paste0("The total area of substrate data within MPAs in Ha: ", round(total_scsr_ha_sub)))


# Summarize total area by substrate category and calculate percentage
substrate_area <- scsr_substrate %>% 
  ...
  ungroup() %>% 
  # Find percentage
  mutate(percent_of_total = (total_area_ha / total_scsr_ha_sub ) * 100) %>%
...
# Plot the data
ggplot(substrate_area, aes(x = cmecs_sc_category, y = percent_of_total, fill = cmecs_sc_category)) +
...
```{r}
# Save plot
ggsave("scsr.png", plot = p1, width = 12, height = 8, bg = "transparent")
```
